...
def main() -> int:
    ...
    def run(args, **kwargs):
        return subprocess.run(args, cwd=ROOT, env=env, check=True, timeout=900, **kwargs)
    ...
    with tempfile.TemporaryDirectory(prefix="muv-docker-check-") as directory:
        ...
        try:
            ...
            print("PASS: container tests, app/DB health, account config and volume persistence.", flush=True)
        finally:
